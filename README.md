...
В интерфейсе кнопка **"Открыть последний HTML-отчёт"** отображается всегда: если отчёт ещё не сформирован, кнопка будет неактивна и появится подсказка.

- UI запускает Playwright с репортерами `list,html,json`, чтобы одновременно сохранять HTML-отчёт и данные для истории запусков.
